...
export default function ProjectScreen({ currentProject, isEditingPermissions, setIsEditingPermissions }) {
	return (
		<Container>
			{currentProject && <TaskList currentProject={currentProject} />}
			<EditPermissionsModal isEditingPermissions={isEditingPermissions} setIsEditingPermissions={setIsEditingPermissions} />
		</Container>
	);
}
...
function TaskList({ currentProject }) {
	...
	const { draftTask, createDraftTask, deleteDraftTask, setDraftTaskName, submitDraftTask } = useDraftTask({ addTask });
	...
	return loading ? (
		...
		<>
			...
			<TaskDetailModal project={currentProject} task={selectedTask} unselectTask={setSelectedTaskId} />
		</>
	);
}
